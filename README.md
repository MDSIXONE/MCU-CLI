...
### 1. 列出串口
```bash
python mcucli/mcucli.py serial list
```

### 2. 重置 MicroPython REPL（重要！）
```bash
python mcucli/mcucli.py serial reset COM9
```

### 3. 发送命令
```bash
python mcucli/mcucli.py serial send COM9 "print('Hello')"
```

### 4. 监控串口
```bash
python mcucli/mcucli.py serial monitor COM9 --duration 10
```
